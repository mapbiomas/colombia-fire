...
def convert_to_raster(dataset_classify, image_data_scene, output_image_name):
    ...
    # **Add creation options for compression and change the data type**
    options = [
        'COMPRESS=DEFLATE',
        'PREDICTOR=2',
        'TILED=YES',
    ...
    # **Make sure the data is of type uint8**
    ...
    while attempt < max_attempts:
        try:
            ...
            with rasterio.open(image) as src:
                ...
                # Reproject the geometry to the image CRS
                geom_shape = shape(geom)
                ...
                # Check for a significant intersection
...
# Function to upload a file to GEE with metadata and check whether the asset already exists
...
# The rest of the code in TensorFlow 1.x style
def create_model_graph(hyperparameters):
    """
    Dynamically builds and returns a TensorFlow computational graph based on the model parameters.
    """
    graph = tf.Graph()
    ...
    with graph.as_default():
        ...
        # Build the neural network layers with the defined hyperparameters
        ...
        # Define the loss function (for training, although it is not needed for inference)
        cross_entropy = tf.reduce_mean(
            tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=y_input),
            name='cross_entropy_loss'
        )
        
        # Define the optimizer (for training, although it is not needed for inference)
        ...
        # Operation to get the predicted class
...
def process_year_by_satellite(satellite_years, bucket_name, folder_mosaic, folder_temp, suffix,
    ...
    for satellite_year in satellite_years[:1 if simulate_test else None]:  # only 1 satellite when testing
        satellite = satellite_year['satellite']
        years = satellite_year['years'][:1 if simulate_test else None]     # only 1 year when testing
        ...
        with tqdm(total=len(years), desc=f'Processing years for satellite {satellite.upper()}') as pbar_years:
            for year in years:
                ...
                if input_scenes:
                    ...
                    # ⏱ Wait up to 10s for the file to be created
                    wait_time = 0

# last_update: '2025/06/02', github:'mapbiomas/colombia-fire', source: 'IPAM', contact: 'contato@mapbiomas.org'
...
def simulate_processing_click(b):
    """
    Simulates the burned area classification processing based on the selected models and mosaics.

    It prepares the list of objects the same way as `classify_burned_area_click`, but only runs the simulation,
    without doing the real processing.
    """
    selected_models = collect_selected_models()  # Collect the models selected in the checkboxes
    models_to_simulate = []  # This list will hold the objects for the simulation

    if selected_models:
        for model in selected_models:
            log_message(f'Simulando o processamento do modelo: {model}')

            # Normalize the model name to make sure the right format is used
            model_key = f"{model}.meta" if not model.endswith('.meta') else model

            # Check that the mosaics associated with the model are there
            if model_key in mosaic_checkboxes_dict:
                mosaic_checkboxes = mosaic_checkboxes_dict[model_key]

                # Collect the selected mosaics
                ...
                # Build the model object for the simulation
                model_obj = {
                    "model": model,  # Model name
                    "mosaics": selected_mosaics,  # List of selected mosaics
                    "simulation": True  # Flag as simulation
                }
        ...
        # If there are models to simulate, call render_classify_models
        if models_to_simulate:
            log_message(f'Chamando render_classify_models para simulação com: {models_to_simulate}')
            render_classify_models(models_to_simulate)  # Call the function with the list prepared for simulation
        else:
...
def classify_burned_area_click(b):
    ...
    if selected_models:
        for model in selected_models:
            ...
            country = parts[1]  # For example, 'colombia'

# last_update: '2025/04/24', github:'mapbiomas/colombia-fire', source: 'IPAM', contact: 'contato@mapbiomas.org'
...
# Function to check for and install libraries
...
# Define the directories for data storage and model output
folder = f'/content/mapbiomas-fire/sudamerica/{country}'  # Main directory where the data is stored

folder_samples = f'{folder}/training_samples'  # Directory for sample data storage
folder_model = f'{folder}/models_col1'  # Directory for model output storage
folder_images = f'{folder}/tmp1'  # Directory for temporary image storage
...
class ModelTrainer:
    def __init__(self, bucket_name, country, folder_model, get_active_checkbox_func):
        ...
        self.get_active_checkbox = get_active_checkbox_func  # existing global function
...
class ImageProcessor:
    ...
    def convert_to_array(self, dataset):
        ...
        data = np.transpose(data, (1, 2, 0))  # To (height, width, bands)
        return data
...
# Optimized function to remove NaNs and shuffle the data
def filter_valid_data_and_shuffle(data):
    """Remove rows with NaN and shuffles the data, optimized."""
    # Remove NaNs in a vectorized way, without loops
    mask = np.all(~np.isnan(data), axis=1)  # Build a mask that flags the valid rows
    valid_data = data[mask]  # Apply the mask to keep the valid rows

    # Shuffle the data efficiently
    np.random.default_rng().shuffle(valid_data)  # Use the faster Generator for shuffling
    return valid_data
...
# ====================================
# Build the interface and keep it in a global variable

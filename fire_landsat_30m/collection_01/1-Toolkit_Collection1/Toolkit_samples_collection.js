/** @description Toolkit for collection and evaluation of collections used in classifications of collection 2 of MapBiomas-Fogo.
  ...
  * reg
   ...
   stable version: 30/06/2025 - https://code.earthengine.google.com/b74e3bd2100ed8bbc064573c5db50fc0
...
var year = 2024;
var table = 'Countries';//'Countries'; 'Regiones Paraguay'
var region = 'Colombia';// 'paraguay_r6'
//'Bolivia','Chile','Colombia','Paraguay','Peru', 'Ecuador'


// --- --- ----OPTIONS
// --- --- The 'options' object is used as the main cache memory of this toolkit,
...
var options = {
  ...
  // scale used in the area computation
  scaleArea:1000,
};
...
//*/
  ...
  // TYPE 1: Verified with FIRMS thermal anomaly data

  {
    'dataset': 'reference_data',   
    'name': 'ref: Ecuador BA (anomalías térmicas FIRMS)',
    'id': 'projects/mapbiomas-ecuador/assets/FIRE/AUXILIARY_DATA/REFERENCES/cicatrizes_fuego_reference_2020_2025',
    ...
    allProcess: function(col) {
      ...
      years.forEach(function(year) {
        months.forEach(function(month) {
          ...
          var img = ee.Image("projects/mapbiomas-ecuador/assets/FIRE/AUXILIARY_DATA/REFERENCES/cicatrizes_fuego_reference_2020_2025/cicatriz_fuego_FIRMS_" + year);
  ...
  // TYPE 2: Verified by time series analysis
  {
    'dataset': 'reference_data',   
    'name': 'ref: Ecuador BA (series temporales)',
    'id': 'projects/mapbiomas-ecuador/assets/FIRE/AUXILIARY_DATA/REFERENCES/cicatrizes_fuego_reference_2020_2025',
    'years': [2020, 2021, 2022, 2023, 2024, 2025],
    'shown': false,
    'vis': {
    ...
    allProcess: function(col) {
      ...
      years.forEach(function(year) {
        months.forEach(function(month) {
          ...
          var img = ee.Image("projects/mapbiomas-ecuador/assets/FIRE/AUXILIARY_DATA/REFERENCES/cicatrizes_fuego_reference_2020_2025/cicatriz_fuego_SeriesTemporales_" + year);
...
/*  {
  ...
  {
    'dataset':'mapbiomas_col1_versions',
    'id':'projects/mapbiomas-ecuador/assets/FIRE/COLLECTION1/CLASSIFICATION/burned_area_ecuador_v96',
    'name':'MapB Fuego Ecuador v96',
    'shown':false,
    'years':[2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025],
    'vis':{
        min:1,
...
// --- MAPBIOMAS WATER SURFACE
      ...
      //         .filter(ee.Filter.eq("cadence","annual"));
  {
    ...
    'id':'projects/mapbiomas-bolivia/assets/WATER/COLLECTION-3/01-SURFACE/E03-INTEGRATION/water-integracion-01',
    'name':'Superficie del Agua de Bolivia',
    ...
    allProcess:function(col){
      col = ee.ImageCollection(col).filter(ee.Filter.eq("cadence","annual"))
        .map(function(img){
          return img.set({
            'system:time_start':ee.Date(ee.String(img.getNumber('year').int()).cat('-01-01')).millis(),
            'system:time_end':ee.Date(ee.String(img.getNumber('year').add(1).int()).cat('-01-01')).millis()
          });
        });
      return col;
      // return ee.ImageCollection(col.aggregate_array('year').sort()
      //   .iterate(function(current,previous){
      //     var year = ee.Number(current).int();
      //     var start = ee.Date(ee.String(year).cat('-01-01')).millis();
      //     var end = ee.Date(ee.String(ee.Number.parse(year).add(1)).cat('-01-01')).millis();

      //     var image_year = col.filter(ee.Filter.eq('year',year)).mosaic();
...
//
/* // General, merge of the collections of each country
  {
    'dataset':'reference_data',
    'id':'',
...
//*/
  // one land use and cover layer per country
  //
   {
    'dataset':'cobertura_y_uso_de_la_tierra',
  ...
  {
    'dataset':'cobertura_y_uso_de_la_tierra',
    'id':'projects/mapbiomas-public/assets/paraguay/lulc/collection3/mapbiomas_paraguay_collection3_coverage_v1',
    ...
    allProcess:function(image){
      ...
      ////////////////////////////
      var landcover_paraguay  = ee.Image('projects/mapbiomas-public/assets/paraguay/lulc/collection3/mapbiomas_paraguay_collection3_coverage_v1');
...
//*/
  ...
  // --- static vector data used in the spatial filters
  {
    'dataset':'layers',
...
/*  {
  ...
  {
    'dataset':'layers',
    'id':'users/Merino/caiman_pol_certo', // the simplest way to add new spatial filters is to change the table address
    'name':'Case study', // -> CASE STUDY
    allProcess:function(col){
     return col.map(function(feature){
        return feature.set({
...
// --- the 'corrections' functions aim to wrap up all the processing needed for the mosaics
...
// function based on https://developers.google.com/earth-engine/tutorials/community/sentinel-2-s2cloudless
...
//         .gte(1);        
//     }else {
//       // For other months, fetch fires from the previous months of the same year.
//       fire_monitor = ee.ImageCollection('projects/mapbiomas-workspace/FOGO/MONITORAMENTO/collection-fire-monthly-sentinel2-v3')
//         .filter(ee.Filter.eq('year', year))
//         .filter(ee.Filter.lt('month', month)) // Use lt (less than) to select previous months.
//         .sum()
//         .gte(1);
//     }
...
// Adds the NBR band following the formula at https://www.usgs.gov/landsat-missions/landsat-normalized-burn-ratio
// with a few edits so it can be applied to the minimum-NBR quality mosaics
function addBand_NBR (image){
  var exp = '( b("nir") - b("swir2") ) / ( b("nir") + b("swir2") )';
  var minimoNBR = image
    .expression(exp)
    // -> in the USGS formula the scars take the lowest values, so we multiply the result by -1 so that
    // the scars take the maximum values used as reference when processing the quality mosaics
    .multiply(-1)
    // -> legacy adjustments
    .add(1)
    .multiply(1000)
...
// --- The timeFlag and address functions take advantage of the band information not being shuffled when
// processing quality mosaics, to stamp flags on the pixels that allow managing qualitative information
// per pixel. They were first developed to speed up the map click event, and similar functions to timeFlag
// were also used to compute the fire's month of origin in collection 1 of MapBiomas-Fogo

// - stamping the origin of the pixels in landsat images
...
// - recovering the origin address of the pixels in landsat images
...
// - stamping the origin of the pixels in sentinel images
// fix -> the sentinel timeFlag and address functions need the dictionaries used for the translation
// "code_to_number" and "number_to_code" updated with an expansion of codes for South America; currently there are only 1524 keys, all for Brazil
function  timeFlag_sentinel (image) {
  ...
  // split                T               T      _T           same in every image
  // split        20170328                                    propertie_0     -   8 characters
  // split                 083601                             propertie_1     -   6 characters
  // split                        20170328                    propertie_2     -   8 characters
  // split                                 084228             propertie_3     -   6 characters
  // split                                         35RNK      propertie_4     -   converted to decimal, taking 4 characters
  //                                                 
  // var other_flag = {
...
// - recovering the origin address of the pixels in sentinel images
function address_Sentinel (point,image){
    ...
    // split                        20170328                    propertie_2     -   8 characters
    // split                                 084228             propertie_3     -   6 characters
    // split                                         35RNK      propertie_4     -   converted to numeric, taking 4 characters
...
// ---- ---- interface
// ---- helper functions and variables to manage the software interface
var styles = {
  'comum':{
  ...
  // -> this is not a style dictionary but the thumbnail rendering parameters
  'thumbParams':{
...
function setLayout (){
  ...
  // - base
  options.mapp = ui.root.widgets().get(0); // -> capturing the main Map
...
function returnThumbnail (name, image,vis,shown){
  ...
  var exp_mosaic = ui.Button({
    ...
    onClick:function(){
      ...
      // Capture only the first and second parts after the "_"
...
function applyCloudCoverByCountry(col, obj) {
  ...
  // only applies if it is Landsat and the property exists
  var isLandsat =
...
function plotCollection (obj){
    ...
    var switchs = {
        ...
        // excluding layers that have the same name and belong to other years
        options.mapp.layers()
        ...
        // redundancy: for some reason the forEach only runs over half of the event list
        options.mapp.layers()
...
function setNewSubtitle (dataset_string,panel,position){
    ...
    var boolean = options['boolean '+ dataset_string] || '>'; // collapsed
...
// - main function of the control panel interface, responsible for installing and updating the layer toggle widgets.
// It is also used to manage which datasets go into the interface
function setSubtitle (){
  ...
  setVectors();
  // setNewInfoCache();
  // plot the optical satellite image collections
  ...
  // plot the monthly reference fire scar image collections
  ...
  // plot the land use and cover image collections
...
function setSelectRegion(){
  ...
  // filtering vector data for the start of the reference code
  ...
  var samples_b = [
    {
      ...
      var select = ui.Select({
          ...
          // placeholder:'null',
          value:'projects/mapbiomas-guyana/assets/FIRE/COLLECTION1/SAMPLES',
          onChange:function(value){
...
// --- adds a checkbox in the control panel for every layer added to the map
...
function start (){
  ...
  enjoyLayers(); // adds to the panel the layers that were added the traditional way in the code
...
// You can add layers before the start so they show up in the control panel. This is possible thanks to the .enjoyLayers() function
//Map.addLayer(ee.Image("projects/mapbiomas-paraguay/assets/FIRE/COLLECTION2/CLASSIFICATION/burned_area_paraguay_v1/burned_area_paraguay_l89_v1_region3_2025").selfMask(),{},'paraguay_r4_2025');
//var grilla = ee.FeatureCollection('projects/ee-hendatamapbiomasfire/assets/cuadricula_30km_py');
//Map.addLayer(grilla.style({color: '000000', fillColor: '00000000', width: 3}),{},'Grilla');

var grilla = ee.FeatureCollection("projects/mapbiomas-raisg/DATOS_AUXILIARES/VECTORES/ECUADOR/grillaEcuador5km");
Map.addLayer(grilla.style({color: '000000', fillColor: '00000000', width: 3}),{},'Grilla',false);
// - start 
